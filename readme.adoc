...
| field parameters | all properties can be used as filtering (exact/list) input parameters, will be turned into Cypher parameters | { City(name:"Berlin") {name,geo} }
| query parameters | passed through as Cypher parameters | query UserQuery ($name: String!) { User(name:$name) {name,age} }
| relationships | via a @relationship annotated field | type User { name: String, city : City @relation(name:"LIVES_IN", direction:OUT) }
| ordering | via an extra `orderBy` parameter | query UserSortQuery { User(orderBy:[name_desc,age_desc]) {name,age}}
| IDL | define schema via IDL | :POST /graphql/idl "type Person {name: String!, born: Int}"
| Mutations | create/delete mutations inferred from the schema | createMovie(title:ID!, released:Int) / createMoviePersons(title:ID!,persons:[ID!])
...
| Cypher updates | Custom mutations by executing cypher | createPerson(name: String) : Person @cypher(statement:"CREATE (p:Person {name:{name}}) RETURN p")
