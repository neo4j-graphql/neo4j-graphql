...
You should be able to see your schema in the _Docs_ section of GraphiQL.

You will also get auto-completion, validation and hints when writing queries.
...
* We also provide a `filter` argument for more complex filtering with nested predicates.
...
----
query Nineties($released: Long, $firstLetter: String) {

  Movie(released: $released,
        filter: {title_starts_with: $firstLetter}) {
    title
    released
    actors(first: 3) {
      name
...
mutation {
    ...
    cb: createPerson(name:"Chadwick Boseman", born: 1977)
...
Directives `@name` can be used to augment the schema with additional meta-information that we use for processing.
...
type QueryType {
  ...
  coActors(name:ID!): [Person] @cypher(statement:"MATCH (p:Person {name:$name})-[:ACTED_IN]->()<-[:ACTED_IN]-(co) RETURN co")
}
----
...
[source,cypher]
----
CALL graphql.execute({ Person(born: 1961) { name, born } })
----
...
To deconstruct results from a procedure call you can use Cypher features like `UNWIND`, dot, or list expressions
