...
If your Neo4j Server runs with auth enabled, add the appropriate Basic-Auth header in the "Edit HTTP Headers" screen. Make sure the request type is set to `GET`, instead of the default `POST`.
